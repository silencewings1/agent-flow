...
class Checkpointer:
    def __init__(self, db_path: str = ":memory:"):
        ...
        # —— activity 缓存：以 (thread_id, node, activity_key) 为键缓存 LLM 等调用 —— #
        self._conn.execute(
            """
            CREATE TABLE IF NOT EXISTS activity_results (
                ...
                PRIMARY KEY (thread_id, node, activity_key)
            )
            """
        )
        self._conn.commit()

    def put_activity(self, thread_id: str, node: str, activity_key: str,
                     result: Any, status: str) -> None:
        with self._lock:
            self._conn.execute(
                "INSERT OR REPLACE INTO activity_results VALUES (?,?,?,?,?,?)",
                (thread_id, node, activity_key,
                 json.dumps(result, ensure_ascii=False),
                 status, time.time()),
            )
            self._conn.commit()

    def get_activity(self, thread_id: str, node: str,
                     activity_key: str) -> Optional[tuple[Any, str]]:
        row = self._conn.execute(
            "SELECT result, status FROM activity_results "
            "WHERE thread_id=? AND node=? AND activity_key=?",
            (thread_id, node, activity_key),
        ).fetchone()
        if row is None:
            return None
