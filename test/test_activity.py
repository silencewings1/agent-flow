...
def make_activity_node(name: str, key: str, return_value: str = "hello",
    ...
    if counter is None:
        counter = {}
    def fn(state, ctx):
        result = ctx.activity(key, lambda: _do(name, key, return_value, counter))
...
def test_first_call_executes_fn():
    ...
    assert counter.get(("a", "x"), 0) == 1, f"fn 应执行 1 次，实际 {counter.get(('a', 'x'), 0)}"
...
def test_cached_on_resume():
    ...
    def gate_node(state, ctx):
        ...
        result = ctx.activity("llm", lambda: _do("gate", "llm", "cached_val", counter))
    ...
    assert counter.get(("gate", "llm"), 0) == 1, f"首次 fn 应执行 1 次"

    # 恢复执行：gate 重入，activity 应命中缓存
    r2 = app.invoke({}, thread_id="t_resume", command=Command(resume="yes"))
    assert r2.status == "completed"
    # 关键断言：fn 仍然只执行 1 次（命中缓存）
    assert counter.get(("gate", "llm"), 0) == 1, f"恢复后 fn 不应再执行，实际 {counter.get(('gate', 'llm'), 0)}"
...
def test_different_keys_independent():
    ...
    def multi_key_node(state, ctx):
        ...
        r2 = ctx.activity("key_b", lambda: _do("mk", "key_b", "val_b", counter))
        return {"r1": r1, "r2": r2}
    ...
    assert r.status == "completed"
    assert counter.get(("mk", "key_a"), 0) == 1
    assert counter.get(("mk", "key_b"), 0) == 1
    assert r.state["r1"] == "val_a"
    assert r.state["r2"] == "val_b"

    # 再跑一次同一个 thread（节点重入），两个 key 都应命中缓存
    r2 = app.invoke({}, thread_id="t_keys")
    assert counter.get(("mk", "key_a"), 0) == 1, "key_a 缓存命中"
    assert counter.get(("mk", "key_b"), 0) == 1, "key_b 缓存命中"
...
def test_different_threads_independent():
    ...
    def simple_node(state, ctx):
        tid = state.get("tid", "?")
        r = ctx.activity("llm", lambda: _do("tn", "llm", f"thread_{tid}", counter))
        return {"result": r}
    ...
    assert counter.get(("tn", "llm"), 0) == 2, f"两个 thread 应各执行一次，实际 {counter.get(('tn', 'llm'), 0)}"
...
def test_exception_is_cached():
    ...
    # 验证异常类型被保留为 ValueError 而非 RuntimeError
    assert r1.state.get("error_type") == "ValueError", f"异常类型应保留，实际 {r1.state.get('error_type')}"
    ...
    assert r2.state.get("error_type") == "ValueError", f"异常类型应保留，实际 {r2.state.get('error_type')}"
...
def test_activity_without_checkpointer():
    ...
    result = ctx.activity("x", lambda: _do("nc", "x", "direct", counter))
    assert result == "direct"
    assert counter.get(("nc", "x"), 0) == 1
